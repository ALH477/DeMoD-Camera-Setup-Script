...
def tui_menu() -> None:
    ...
    if not cams:
        ...
        if not framerate.isdigit():
            print("Invalid framerate. Using default 30.")
            framerate = '30'
        bitrate = input("Enter bitrate (default 800k): ") or '800k'
        if not bitrate:
            print("Invalid bitrate. Using default 800k.")
            bitrate = '800k'
    ...
    if start == 'y':
        try:
            ...
            print("Error: MediaMTX binary not found. Run setup.sh to install.")
            sys.exit(1)
    ...
    # Rating with questions
    print("\nAnswer these for a security rating:")
    questions = [
