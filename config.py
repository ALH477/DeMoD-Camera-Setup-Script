...
<body>
  ...
  <p>{rating_message}</p>
  {security_status}
  <p>Configure cameras below. Use JSONC for multi-camera setups or form fields for a single camera. Answer security questions to improve your rating.</p>
  <form method="POST">
...
"""
    ...
    def do_POST(self):
        ...
        try:
            ...
            if not cams:
                # Single camera from form
                device = params.get('device', ['/dev/video0'])[0].strip()
                if not device:
                    raise ValueError("Device cannot be empty. Specify a valid path (e.g., /dev/video0).")
                ...
                if not bitrate:
                    raise ValueError("Bitrate cannot be empty (e.g., 800k).")
                enable_auth = 'enable_auth' in params
                auth = {}
                if enable_auth:
                    auth['user'] = params.get('rtsp_user', [''])[0].strip()
                    if not auth['user']:
                        raise ValueError("Username required if authentication is enabled.")
                    auth['pass'] = params.get('rtsp_pass', [''])[0].strip()
                    if not auth['pass']:
                        raise ValueError("Password required if authentication is enabled.")
                cams = [{'device': device, 'framerate': framerate, 'bitrate': bitrate, 'auth': auth}]

            write_yml(cams)

            response = f"Configuration saved for {len(cams)} camera(s).<br>"
            ip = run_command("hostname -I | awk '{print $1}'")
            if "Error" in ip:
                ip = "<your-orange-pi-ip>"
                response += f"Could not determine IP: {ip}.<br>"
            ...
            response += "Streams available at: " + ", ".join(stream_urls) + "<br>"
            ...
            if 'start_server' in params:
                ...
                if MEDIAMTX_PROC and MEDIAMTX_PROC.poll() is None:
                    ...
                    MEDIAMTX_PROC.wait()
                    response += "Previous server stopped.<br>"
                try:
                    MEDIAMTX_PROC = subprocess.Popen(['./mediamtx'])
                    response += "Server started.<br>"
                except FileNotFoundError:
                    response += "Error: MediaMTX binary not found. Run setup.sh to install.<br>"

            # Updated status and rating
            checks = security_checks()
            user_answers = [
